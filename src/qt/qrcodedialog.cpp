...
{
    QString ret = QString("stlcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non STL unit input in lnReqAmount, we generate the URI with STL as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::STL, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
